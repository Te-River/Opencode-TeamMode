...
> **On OpenCode 2.x, four of those tools are deliberately not registered.**
...
| `TM_V2_CODEMODE` | unset (catalog) — **v2 only** | How our tools reach the model. OpenCode 2.x decides tool visibility with `options.codemode`: unset means we send no flag, so `tm_*` are offered only through the host's Code Mode catalog with ≤120 chars of their first description line — callable from inside `execute`, but without the governance text we wrote. `direct` sends `codemode:false`, delivering the full definitions at a measured cost per request: build-class specialists ≈2 660 tokens, tester ≈5 448, researcher ≈7 113, the lead ≈8 515. The boot log and `tm_stats` say which of the two you are in |
