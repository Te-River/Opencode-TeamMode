/**
 * The v1 personality: everything this plugin does through the OpenCode 1.18.x
 * `server(input, options)` hook object.  Moved out of `src/index.ts` verbatim
 * for the v2 port (see docs plan) — no behaviour change is allowed here; a v1
 * host must observe byte-identical hooks, and the ~1700 assertions in
 * test-*.mjs are the oracle that keeps that promise.
 *
 * v1-only by construction: the single composed `tool.execute.before` slot, the
 * permission-event approval gate, the `{"*": "ask"}` bash escalation object,
 * and `tui.showToast` notification.
 */

import type { OpenCodeConfig, PermissionEvent, HostEvent, PluginInput, Hooks } from "../types.js"
...
export async function createV1Personality(
  input: PluginInput,
