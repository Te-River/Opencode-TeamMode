...
| `TM_V2_CODEMODE` | 不设（目录模式）——**仅 v2** | 我们的工具怎么送到模型面前。OpenCode 2.x 用 `options.codemode` 决定可见性：不设置就是我们不发这个标记，`tm_*` 只出现在宿主的 Code Mode 目录里，只保留描述首行 ≤120 字符——能在 `execute` 里调用，但我们为它们写的治理文案送不到。`direct` 会发 `codemode:false`，把完整定义交出去，代价是按角色实测过的：build 类专家 ≈2 660 token、tester ≈5 448、researcher ≈7 113、领队 ≈8 515。启动日志和 `tm_stats` 会说清楚当前是哪一种世界 |
