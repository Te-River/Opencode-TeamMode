...
| 🧪 **Tester** | 测试工程师 | 单元测试、集成测试、边界覆盖、UI 验证 |
...
### Ultra Review、流水线门禁与证据标准（v1.4.6）

- **Ultra Review**：每个非平凡变更由**三个并行 Reviewer 派单**审查，各管一个
  维度——完整性（需求覆盖）/ 正确性（逻辑与安全）/ 影响面（回归与波及面）——
  且各自被明令忽略其余两维。Lead 先把三份报告合并去重成一份按严重度分组的
  清单，再进入修复闭环。平凡变更可跳过审查，但须一行理由。
- **流水线门禁**：调研全部完成才定计划；设计先行于实现；实现归队才测试；
  测试通过才审查；用户可见的前端改动必须通过 UI 验证才算完成；最终报告
  等所有代理收工、看板状态属实。平凡任务跳过阶段须一行理由。
- **证据标准**："完成 / 修复 / 通过"的声明必须附可验证证据——命令输出、
  日志、diff、截图。纯叙述只是进度说明，不是证明（对子代理和 lead 同样生效）。
- **README 优先**：lead 在规划前先读项目 README（及 AGENTS.md / CLAUDE.md），
  并把有约束力的约定复述进每次派单；子代理对清单内项目约定视同强制。
- **逐字契约**：需要互通的并行实现代理，其数据契约（端点、字段名、类型）只写
  一份并**逐字**贴进每个相关派单——契约错配是集成 bug 之首。
- **CHANGELOG 维护**：项目带有 CHANGELOG.md 时，交付的改动会按
