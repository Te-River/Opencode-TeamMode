...
| `TM_V2_CODEMODE` | **direct（默认）**——**仅 v2** | 我们的工具怎么送到模型面前。OpenCode 2.x 用 `options.codemode` 决定可见性：默认发 `options.codemode:false`，所以工具以真正的定义到达模型，`tm_*` 只出现在宿主的 Code Mode 目录里，只保留描述首行 ≤120 字符——能在 `execute` 里调用，但我们为它们写的治理文案送不到。`direct` 会发 `codemode:false`，把完整定义交出去，代价是按角色实测过的：代价按角色实测：build 类专家 ≈2 660 token、tester ≈4 726、researcher ≈6 391、领队 ≈7 366。启动日志和 `tm_stats` 会说清楚当前是哪一种世界 |
