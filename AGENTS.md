...
| `test-tm-tools.mjs` | JIT tool assertions (threshold boundary, previews, HMAC/refs, P3, R6 reuse, degradation, trajectory append-only) + §9 PTC (config, budgets, arg schema, staticPscan, five statuses + retry, composite step numbers, trajectory shapes, summary shape pin, tool registration with five-tool set) |
