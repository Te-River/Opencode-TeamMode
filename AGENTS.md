...
| `src/tm/` | JIT layer-2 tools (tm_read/tm_grep/tm_bash/tm_fetch/tm_webfetch) -- `config.ts` env knobs + CJK-aware token口径 (CJK ≈ 1 token, other chars/4; `capTokens` truncates by the same cost basis), git-aware store dirs (AUTO = `<repo>/.git/opencode-team/{blackboard,trajectory}`, tmpdir fallback; explicit `TM_*_DIR` keeps absolute/project-relative semantics), `refs.ts` run ids + HMAC handles, `store.ts` run store + append-only trajectory + TTL sweep, `preview.ts` 5-branch previews (80-token cap) + structure summaries, `guard.ts` P2 path scope + P3 readonly allowlist (rejections guide to the bash confirmation dialog), `tools.ts` governed pipelines + `buildPtcArgsSchema()`/`buildWebfetchArgsSchema()` + shared-instance injection (`buildTmTools(deps, pipelines?)`), `webfetch.ts` governed web fallback (domain allowlist seeded with moegirl/bilibili/bing/baidu, `TM_WEBFETCH_ALLOWED_DOMAINS` override, http(s)-only, per-hop redirect re-check, remote env-file URL red line, HTML→text, 2 MB cap, 20 s timeout), `index.ts` assembly (ONE main pipeline instance shared by the four tools + tm_webfetch; PTC gets a `ptc-` stepPrefix so its step ids can't collide) + `tm_ptc_run` registration (M3) |
...
| `test-tm-tools.mjs` | JIT tool assertions (threshold boundary, previews, HMAC/refs, P3, R6 reuse, degradation, trajectory append-only, §6k shared $ shell-bridge fallback, §6m tm_webfetch allowlist/red-lines/offload) + §9 PTC (config, budgets, arg schema, staticPscan, five statuses + retry, composite step numbers, trajectory shapes, summary shape pin, tool registration with six-tool set, §9j REAL engine runs: WorkerEngine RPC/throw/terminate/env-isolation/strict + InlineVmEngine compile-timeout kill) |
